...
APK="${1:-$ROOT/dist/FramiltonChess-v3-offline-arm64.apk}"
...
with zipfile.ZipFile(apk) as archive:
    ...
    assert manifest["package"] == "com.framilton.knight", manifest
    assert manifest["application_label"] == "Framilton Chess", manifest
    assert manifest["activity_label"] == "Framilton Chess", manifest
    ...
    assert b"product=Framilton Chess" in metadata_bytes
    assert b"package=com.framilton.knight" in metadata_bytes
