...
pause_on_error() {
  code=$?
  echo
  echo "Framilton Chess v3 stopped with exit code $code."
...
./scripts/verify-apk.sh "$ROOT/dist/FramiltonChess-v3-offline-arm64.apk" "$ROOT/stockfish-android-arm64-universal.zip"
...
echo
echo "Framilton Chess v3 is ready:"
echo "$ROOT/dist/FramiltonChess-v3-offline-arm64.apk"
if (( ! PUBLISH )); then
